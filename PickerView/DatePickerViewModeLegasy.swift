...
//        print(x)
    ...
    private func getNextDay(_ day: Date) -> Date {
        ...
        guard let result = Calendar.current.date(byAdding: oneDay, to: day) else {
            fatalError()
        }
        return result
    }
    
    private func add(minute: Int, to date: Date) -> Date {
        let minute = DateComponents(minute: minute)
        guard let result = Calendar.current.date(byAdding: minute, to: date) else {
            fatalError()
        }
        return result
    }
    ...
    private func nextBeginHour(_ date: Date) -> Date {
    ...
    private func addFiveMinute(_ day: Date) -> Date {
        let minute = 5 * 60
        return day + TimeInterval(minute)
    }
